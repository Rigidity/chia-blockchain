...
class Mempool:
    ...
    def __init__(self, mempool_info: MempoolInfo, fee_estimator: FeeEstimatorInterface):
        ...
        with self._db_conn:
            ...
            self._db_conn.execute(
                "CREATE INDEX assert_before_height ON tx(assert_before_height) "
                "WHERE assert_before_height IS NOT NULL"
            )
            self._db_conn.execute(
                "CREATE INDEX assert_before_seconds ON tx(assert_before_seconds) "
                "WHERE assert_before_seconds IS NOT NULL"
            )
